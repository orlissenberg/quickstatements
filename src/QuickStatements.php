...
namespace MagnusManske\MagnusTools;

use Mediawiki\Api\MediawikiApi;
use Toolforge\MediaWikiOAuth;
...
class QuickStatements
{
    ...
    protected $actions_v1 = ['L' => 'label', 'D' => 'description', 'A' => 'alias', 'S' => 'sitelink'];
    ...
    public function __construct()
    {
        ...
        $this->config = json_decode(json_encode(require __DIR__ . '/../config/settings.php'));
        $this->config->logfile = env('LOG_FILE', __DIR__ . '/../logs/quickstatements.log');
    ...
    {
        if (!isset($this->oa)) {
            $toolname = 'quickstatements';

            $params = [
                'tool' => $toolname,
                'language' => $this->getSite()->oauth->language,
                'project' => $this->getSite()->oauth->project,
                'ini_file' => env('MEDIAWIKI_OAUTH_CONFIG_FILE', __DIR__ . '/../config/oauth.ini'),
    ...
    public function setOA(MediaWikiOAuth $oa)
    {
        if (isset($this->oa)) {
    ...
    public function importData($data, $format, $persistent = false)
    {
        ...
        // TODO persistent
        if ($format == 'v1') {
            $this->importDataFromV1($data, $ret);
        } elseif ($format == 'csv') {
            $this->importDataFromCSV($data, $ret);
        } else {
            $ret['status'] = "ERROR: Unknown format $format";
        }
    ...
    public function addBatch($commands, $user_id, $name = '', $site = '')
    {
        ...
        if (count($commands) == 0) {
            return $this->setErrorMessage('No commands');
        }
        if ($this->use_command_compression) {
        ...
        if (!$result = $db2->query($sql)) {
            $this->log("Could not store OAuth information for batch {$batch_id} [{$db->error}]");
        }
        $db2->close();
        ...
        foreach ($commands as $k => $c) {
            ...
            $status = 'INIT';
            if (isset($c->status) and trim($c->status) != '') {
    ...
    function setAuthDbName()
    {
        ...
        if (!$result = $this->db->query($sql)) {
            return $this->setErrorMessage('There was an error running the query [' . $this->db->error . ']' . "\n$sql");
        }
        if ($o = $result->fetch_object()) {
            $this->auth_db = preg_replace('/_p$/', '_auth', $o->db);
        } else {
            return $this->setErrorMessage('There was an error running the query [' . $this->db->error . ']' . "\n$sql");
        }
    }
    ...
    {
        $batch_id *= 1;
        if (isset ($qs_global_bot_api)) {
            unset ($qs_global_bot_api);
        }
        ...
        if ($user_name == '') {
            return $this->setErrorMessage("Cannot determine user name for batch #" . $batch_id);
        }
    ...
    public function fillOA($user_id)
    {
        ...
        $oa = $this->getOA();
        if (isset($oa->gTokenKey) and $oa->gTokenKey != '' and isset($oa->gTokenSecret) and $oa->gTokenSecret != '') {
            return true;
        } # Already set
        ...
        if ($j == '') {
            return false;
        } # No OAuth information for the batch
        $j = json_decode($j);

        if (!isset($j->gTokenKey) or !isset($j->gTokenSecret)) {
            return false;
        } # No info in JSON
    ...
    {
        if (!$this->canCurrentUserChangeBatchStatus($batch_id)) {
            return false;
        }
    ...
    {
        if (false === $this->getCurrentUserID()) {
            return $this->setErrorMessage("Not logged in");
        }
    ...
    protected function ensureCurrentUserInDB()
    {
        ...
        if (!$result = $db2->query($sql)) {
            return $this->setErrorMessage('There was an error running the query [' . $db2->error . ']' . "\n$sql");
        }
    ...
    protected function createNewItem($command)
    {
        ...
        if (isset($command->data)) {
            $data = json_encode($this->array2object($command->data));
        }
        $this->runAction([
            'action' => 'wbeditentity',
            'new' => $command->type,
        ...
        if (!$this->isBatchRun()) {
            $this->wd->updateItem($command->item);
        }
    ...
    {
        if (!$this->isProperty($command->property)) {
            return;
        }
        if (!isset($command->datavalue)) {
            return;
        }
        $q = $command->item;

        $this->wd->loadItem($q);
        if (!$this->wd->hasItem($q)) {
            return;
        }
        $i = $this->wd->getItem($q);
        ...
        foreach ($claims as $c) {
            ...
            if ($c->mainsnak->snaktype === "somevalue" || $c->mainsnak->snaktype === "novalue") {
                ...
                if ($lackingValueType === $command->datavalue->type) {
                    return $c->id;
                }
            } else {
                if (!isset($c->mainsnak) or !isset($c->mainsnak->datavalue)) {
                    continue;
                }
                if (!isset($command->datavalue)) {
                    continue;
                }
                if ($this->compareDatavalue($c->mainsnak->datavalue, $command->datavalue)) {
                    return $c->id;
                }
            }
        }
    ...
    protected function compareDatavalue($d1, $d2)
    {
        ...
        if ($d1->type == 'quantity') {
            return $d1->value->amount * 1 == $d2->value->amount * 1;
        }
        if ($d1->type == 'time') {
            if ($d1->value->calendarmodel != $d2->value->calendarmodel) {
                return false;
            }
            if ($d1->value->precision != $d2->value->precision) {
                return false;
            }
        ...
        if ($d1->type == 'globecoordinate') {
            return $d1->value->latitude == $d2->value->latitude and $d1->value->longitude == $d2->value->longitude and $d1->value->globe == $d2->value->globe;
        }
        if ($d1->type == 'monolingualtext') {
            return $d1->value->text == $d2->value->text and $d1->value->language == $d2->value->language;
        }

        $et = 'entity-type';
        $nid = 'numeric-id';
        if ($d1->type == 'wikibase-entityid') {
            if ($d1->value->$et != $d2->value->$et) {
                return false;
            }
            if (isset($d1->value->$nid) and isset($d2->value->$nid) and $d1->value->$nid == $d2->value->$nid) {
                return true;
            }
            if (isset($d1->value->id) and isset($d2->value->id) and $d1->value->id == $d2->value->id) {
                return true;
            }
    ...
    public function compressCommands($commands)
    {
        ...
        for ($pos = 1; $pos < count($commands); $pos++) {
            ...
            if ($out[count($out) - 1]['action'] == 'create' and $out[count($out) - 1]['type'] == 'item' and $commands[$pos]['action'] == 'add' and strtolower($commands[$pos]['item']) == 'last') {
                if (!isset($out[count($out) - 1]['data'])) {
                    $out[count($out) - 1]['data'] = [];
                }
                if ($commands[$pos]['what'] == 'label') {
                    $lang = $commands[$pos]['language'];
                    $out[count($out) - 1]['data']['labels'][$lang] = [
                        'language' => $lang,
                        'value' => $commands[$pos]['value'],
                    ];
                } else {
                    if ($commands[$pos]['what'] == 'description') {
                        $lang = $commands[$pos]['language'];
                        $out[count($out) - 1]['data']['descriptions'][$lang] = [
                            'language' => $lang,
                            'value' => $commands[$pos]['value'],
                        ];
                    } else {
                        if ($commands[$pos]['what'] == 'alias') {
                            $lang = $commands[$pos]['language'];
                            $out[count($out) - 1]['data']['aliases'][$lang][] = [
                                'language' => $lang,
                                'value' => $commands[$pos]['value'],
                            ];
                        } else {
                            if ($commands[$pos]['what'] == 'sitelink') {
                                $site = $commands[$pos]['site'];
                                $out[count($out) - 1]['data']['sitelinks'][$site] = [
                                    'site' => $site,
                                    'title' => $commands[$pos]['value'],
                                ];
                            } else {
                                if ($commands[$pos]['what'] == 'statement' and isset($commands[$pos]['datavalue'])) {
                                    $claim = [
                                        'mainsnak' => [
                                            'snaktype' => 'value',
                                    ...
                                    while ($pos2 < count($commands)
                                        ...
                                        and in_array($commands[$pos2]['what'], ['qualifier', 'sources'])
                                        ...
                                        if ($commands[$pos2]['what'] == 'sources') {
                                            ...
                                            if ($commands[$pos2]['what'] == 'qualifier') {
                                                $qual = [
    ...
    public function setSite($new_site)
    {
        ...
        if ($this->config->site == $new_site) {
            return true;
        } // All is well
        if (!isset($this->config->sites->$new_site)) {
            return $this->setErrorMessage("Site '{$new_site}' not in config file");
        }
    ...
    {
        if ($this->bot_config_file != '') {
            return parse_ini_file($this->bot_config_file);
        }
    ...
    protected function getBotAPI($force_login = false)
    {
        ...
        if (!isset($this->bot_api) and isset($qs_global_bot_api)) {
            $this->bot_api = $qs_global_bot_api;
        }
        if (!$force_login and isset($this->bot_api) and $this->bot_api->isLoggedIn()) {
            return $this->bot_api;
        }

        $bot_config = $this->getBotConfig();
        $api = new MediawikiApi($this->getAPI());
        if ($force_login or !$api->isLoggedin()) {
            if (isset($bot_config['user'])) {
                $username = $bot_config['user'];
            }
            if (isset($bot_config['username'])) {
                $username = $bot_config['username'];
            }
            if (isset($bot_config['pass'])) {
                $password = $bot_config['pass'];
            }
            if (isset($bot_config['password'])) {
    ...
    public function runBotAction($params_orig, $attempts_left = 1000, $lag = 0)
    {
        ...
        $params = [];
        foreach ($params_orig as $k => $v) {
            $params[$k] = $v;
        } // Copy to array, and for safekeeping original
    ...
    protected function runAction($params, &$command)
    {
        ...
        if (isset($params->summary) and $params->summary != '') {
            $summary .= '; ' . $params->summary;
        } else {
            if (isset($command->summary) and $command->summary != '') {
                $summary .= "; " . $command->summary;
            }
        }
        if ($this->toolname != '') {
            $summary .= "; invoked by " . $this->toolname;
        }
        ...
        if ($this->use_oauth) {
            ...
            $this->log([$params, $status]);
            if (isset($oa->last_res)) {
                $result = $oa->last_res;
            }
        } else {
			...
			*/
            ...
            if (!isset($result) or $result === null or $result == '') {
                ...
                if (isset($result->error) and isset($result->error->info)) {
                    ...
                    $command->message = json_encode($result);
                }
            }
        }
    }
    ...
    protected function getPrefixedID($q)
    {
        ...
        foreach ($this->getSite()->types as $char => $data) {
            if (!isset($data->ns_prefix) or $data->ns_prefix == '') {
                continue;
            }
            if (preg_match('/^' . $char . '\d+$/', $q)) {
                return $data->ns_prefix . $q;
            }
        }
    ...
    protected function commandAddStatement($command, $i, $statement_id)
    {
        ...
        if (isset($statement_id)) {
            return $this->commandDone($command, "Statement already exists as $statement_id");
        }
    ...
    {
        // Paranoia
        if (!isset($command->qualifier)) {
            return $this->commandError($command, "Incomplete command parameters");
        }
        if (!isset($command->qualifier->prop)) {
            return $this->commandError($command, "Incomplete command parameters");
        }
        if (!preg_match('/^P\d+$/', $command->qualifier->prop)) {
            return $this->commandError($command, "Invalid qualifier property {$command->qualifier->prop}");
        }
        ...
        $this->runAction($action, $command);
        if ($command->status == 'error' and preg_match('/The statement has already a qualifier/', $command->message)) {
            $command->status = 'done';
        }
        if (!$this->isBatchRun()) {
            $this->wd->updateItem($command->item);
    ...
    protected function commandAddSources($command, $i, $statement_id)
    {
        ...
        if (count($command->sources) == 0) {
            return $this->commandError($command, "No sources to add");
        }
    ...
    {
        // Paranoia
        if ($i->getLabel($command->language, true) == $command->value) {
            return $this->commandDone($command, 'Already has that label for {$command->language}');
        }
    ...
    {
        // Paranoia
        if ($i->getDesc($command->language, true) == $command->value) {
            return $this->commandDone($command, 'Already has that description for {$command->language}');
        }
    ...
    protected function commandSetSitelink($command, $i)
    {
        ...
        if (isset($sl) and str_replace(' ', '_', $sl) == str_replace(' ', '_', $command->value)) {
            return $this->commandDone($command, 'Already has that sitelink for {$command->site}');
        }
    ...
    public function runSingleCommand($command)
    {
        ...
        if (!isset($command)) {
            return $this->commandError($command, "Empty command");
        }
        ...
        if (isset($this->temporary_batch_id)) {
            if (isset($command->summary)) {
                $command->summary .= ' ' . $this->getTemporaryBatchSummary();
            } else {
                $command->summary = $this->getTemporaryBatchSummary();
            }
        }
        ...
        if ($command->action == 'create') {
            ...
            if ($command->action == 'merge') {
                ...
                // Prepare
                if (!isset($command->item) and isset($command->id) and $command->what == 'statement') {
                    $command->item = strtoupper(preg_replace('/\$.+$/', '', $command->id));
                }
                $q = trim($command->item);
                if (strtolower($q) == 'last') {
                    $q = $this->last_item;
                }
                if ($q == '') {
                ...
                $command->item = $q;
                $to_load = [$q];
                if (isset($command->property) and $this->isProperty($command->property)) {
                    $to_load[] = $command->property;
                }
                $this->wd->loadItems($to_load);
                if (!$this->wd->hasItem($q)) {
                    return $this->commandError($command, "Item $q is not available");
                }
                $i = $this->wd->getItem($q);

                if ($command->action == 'add') {
                    // Do it
                    if ($command->what == 'label') {
                        return $this->commandSetLabel($command, $i);
                    }
                    if ($command->what == 'alias') {
                        return $this->commandAddAlias($command, $i);
                    }
                    if ($command->what == 'description') {
                        return $this->commandSetDescription($command, $i);
                    }
                    ...
                    if ($command->what == 'statement') {
                        return $this->commandAddStatement($command, $i, $statement_id);
                    }
                    ...
                    if ($command->what == 'qualifier') {
                        return $this->commandAddQualifier($command, $i, $statement_id);
                    }
                    if ($command->what == 'sources') {
                        return $this->commandAddSources($command, $i, $statement_id);
                    }
                } else {
                    if ($command->action == 'remove') {
                        if ($command->what == 'statement') {
                            if (!isset($command->id)) {
                                $command->id = $this->getStatementID($command);
                            }
                            if (!isset($command->id) or $command->id == '') {
                                return $this->commandError($command, "Base statement not found");
                            }
    ...
    protected function importDataFromV1($data, &$ret)
    {
        ...
        foreach ($rows as $row) {
            ...
            if (count($cols) >= 3 and ($this->isValidItemIdentifier($first) or $first == 'LAST') and $this->isValidItemIdentifier($cols[1])) {
                ...
                $cmd = ['action' => $action, 'item' => $first, 'property' => $prop, 'what' => 'statement'];
                if ($comment != '') {
                    $cmd['summary'] = $comment;
                }
                ...
                while (count($cols) >= 2) {
                    ...
                    if (preg_match('/^([SP])(\d+)$/i', $key, $m)) {
                        ...
                        $cmd = [
                            ...
                            'property' => $prop,
                            'what' => $what,
                            'datavalue' => $last_command['datavalue'],
                        ];
                        $dummy = [];
                ...
                if (count($cols) != 0) {
                    $cmd['error'] = 'Incomplete reference/qualifier list';
                }
            } else {
                if (count($cols) === 3 and ($this->isValidItemIdentifier($first) or $first == 'LAST') and preg_match('/^([LADS])([a-z_-]+)$/i', $cols[1], $m)) {
                    $code = strtoupper($m[1]);
                    $lang = strtolower(trim($m[2]));
                    $cmd = ['action' => $action, 'what' => $this->actions_v1[$code], 'item' => $first];
                    if ($comment != '') {
                        $cmd['summary'] = $comment;
                    }
                    if ($code == 'S') {
                    ...
                    if ($first == 'MERGE' and count($cols) == 3 and $this->isValidItemIdentifier($cols[1]) and $this->isValidItemIdentifier($cols[2])) {
                        $q1 = $cols[1];
                        $q2 = $cols[2];
                        if (preg_replace('/\D/', '', $q1) * 1 < preg_replace('/\D/', '', $q2) * 1) {
                            [$q1, $q2] = [$q2, $q1];
                        } // Always merge into older item
                        $cmd = ['action' => 'merge', 'type' => 'item', 'item1' => $q1, 'item2' => $q2];
                        if ($comment != '') {
                            $cmd['summary'] = $comment;
                        }
                    } else {
                        if ($first == 'CREATE') {
                            $cmd = ['action' => 'create', 'type' => 'item'];
                            if ($comment != '') {
                                $cmd['summary'] = $comment;
                            }
                        } else {
                            if ($first == 'STATEMENT' and count($cols) == 2) {
                                $id = trim($cols[1]);
                                $cmd = ['action' => $action, 'what' => 'statement', 'id' => $id];
                                if ($comment != '') {
                                    $cmd['summary'] = $comment;
                                }
                            }
            ...
            if (isset($cmd['action']) && !$skip_add_command) {
                $ret['data']['commands'][] = $cmd;
            }
        }
    ...
    protected function getEntityType($q)
    {
        ...
        if (preg_match('/^L\d+-F\d+$/', $q)) {
            return 'form';
        }
        if (preg_match('/^L\d+-S\d+$/', $q)) {
            return 'sense';
        }
    ...
    protected function parseValueV1($v, &$cmd)
    {
        ...
        if ($v == 'somevalue' || $v == 'novalue') {
            $cmd['datavalue'] = ["value" => $v, "type" => $v];

            return true;
        }

        if ($v == 'LAST') {
            $cmd['datavalue'] = ["type" => "wikibase-entityid", "value" => ["entity-type" => "item", "id" => "LAST"]];
        ...
        if ($this->isValidItemIdentifier($v)) {
            $cmd['datavalue'] = [
                ...
                "value" => ["entity-type" => $this->getEntityType($v), "id" => strtoupper($v)],
            ];
        ...
        if (preg_match('/^"(.*)"$/i', $v, $m)) { // STRING
            $cmd['datavalue'] = ["type" => "string", "value" => trim($this->enforceStringEncoding($m[1]))];
        ...
        if (preg_match('/^([a-z_-]+):"(.*)"$/i', $v, $m)) { // MONOLINGUALTEXT
            $cmd['datavalue'] = [
                ...
                "value" => ["language" => $m[1], "text" => trim($this->enforceStringEncoding($m[2]))],
            ];
        ...
        $cmd['datavalue'] = ["type" => "unknown", "text" => $v];
        $cmd['error'] = ['PARSE', 'Unknown V1 value format'];
    }
}