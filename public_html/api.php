...
function validate_origin()
{
    ...
    if (!isset($qs->config->valid_origin)) {
        return;
    }
    if ($qs->config->valid_origin == '') {
        return;
    }
    $valid_origin = $qs->config->valid_origin;
    if (!is_array($valid_origin)) {
        $valid_origin = [$valid_origin];
    }
    $origin = get_origin();
    if (in_array($origin, $valid_origin)) {
        return;
    } // OK
    fin('Invalid origin');
}
            ...
            if ($action == 'is_logged_in') {
                ...
                if ($action == 'get_batch_info') {
                    ...
                    if ($batch == 0) {
                        ...
                        $out['data'] = $qs->getBatchStatus([$batch]);
                    }
                } else {
                    if ($action == 'get_batches_info') {
                        $out['debug'] = $_REQUEST;
                        ...
                        $conditions = [];
                        if ($user != '') {
                            $conditions[] = "user.id=batch.user AND user.name='" . $db->real_escape_string($user) . "'";
                        }
                        ...
                        if ($action == 'get_commands_from_batch') {
                            ...
                            if ($action == 'run_single_command') {
                                ...
                                if ($action == 'start_batch' or $action == 'stop_batch') {
                                    $batch_id = get_request('batch', 0) * 1;
                                    ...
                                    if ($action == 'stop_batch') {
                                        $res = $qs->userChangeBatchStatus($batch_id, 'STOP');
                                    }
                                    ...
                                    if ($action == 'run_batch') {
                                        ...
                                        if ($action == 'get_batch') {
                                            ...
                                            if ($action == 'reset_errors') {
                                                ...
                                                $ids = [];
                                                while ($o = $result->fetch_object()) {
                                                    if (stristr($o->message, 'no-such-entity')) {
                                                        continue;
                                                    } // No such item exists, no point in re-trying
                                                    if (!isset($o->json)) {
                                                        continue;
                                                    } // No actual command
                                                    ...
                                                    if (isset($j->item) and $j->item == 'LAST') {
                                                        continue;
                                                    } // Don't know which item to re-apply for
                                                    if (isset($j->action) and $j->action == 'CREATE' and !isset($j->data)) {
                                                        continue;
