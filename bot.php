...
function iterate()
{
    ...
    while ($o = $result->fetch_object()) {
        ...
        if (!$qs2->runNextCommandInBatch($o->id)) {
            print $qs2->last_error_message . "\n";
        } else {
            $ret++;
        }
...
if (isset($argv[1]) and $argv[1] == 'single_batch') {
    ...
    while ($o = $result->fetch_object()) {
        ...
        if (!preg_match('/^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})$/', $ts_last_change, $m)) {
            continue;
        } #die ( "Bad time format in ts_last_change batch #{$o->id}\n" ) ;
        $ts_last_change = $m[1] . '-' . $m[2] . '-' . $m[3] . ' ' . $m[4] . ':' . $m[5] . ':' . $m[6];
        $diff_sec = time() - strtotime($ts_last_change);
        #print "{$ts_last_change}\n{$diff_sec}\n" ;
        if ($diff_sec < $min_sec_inactive and $o->status != 'INIT') {
            continue;
        } #exit ( 0 ) ; # Oldest batch is still too young
        print "Using {$o->id}\n";
        ...
        while (1) {
            ...
            if ($status[$o->id]['batch']->status != 'RUN') {
                break;
            }
        }
        break;
